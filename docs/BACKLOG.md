...
### REQ-9 · Refactor vendor suspension — ✅ policy decided + implemented (see PR for this branch)
- **Policy, written down as asked:**
  - **New bookings while suspended:** already blocked, and turned out not to be a gap — `bookings.service.ts` already refuses to create a booking unless `vendor.status === 'verified'`, and `vehicles.service.ts`'s public browse already joins on `vendors.status = 'verified'`, so a suspended vendor's cars vanish from search on their own. Confirmed by reading both before writing new code.
  - **Requested (not yet confirmed) bookings:** already auto-declined on suspend (existing code, `admin.service.ts` — a vendor who can't honour a request shouldn't leave a customer waiting on an answer that never comes).
  - **Confirmed / in-progress bookings: left standing, not auto-cancelled.** The customer already committed — travel plans, a deposit — and the vendor's problem isn't theirs to inherit by surprise. Ops reviews and decides case-by-case (cancel + refund vs honour); this was already the deliberate choice in the code's own comment, just never written down here. What *is* new: the affected customer gets a transparency notice by email ("your provider is under review, your booking stands for now") so they're not the last to know, without the message claiming a cancellation that hasn't been decided.
  - **Suspension reason:** now required to suspend at all (`SetVendorStatusDto.reason`), stored on `vendors.suspension_reason` (migration `0027`), shown to the vendor on their dashboard and to admin on the Vendors tab, and cleared on reinstatement (a resolved reason shouldn't linger as if still in force).
  - **Emails:** vendor gets one on suspend (with the reason) and on reinstatement; each customer with a standing booking gets the transparency notice above. All best-effort via the existing `NotificationsService` — a failed send never blocks the status change, consistent with every other email in this app. Real delivery still depends on REQ-4 (Resend domain verification) landing.
- **Where it landed:** `packages/shared` (`Vendor.suspension_reason`, `PublicVendor` omit), `apps/api/src/admin` (`dto.ts`, `admin.service.ts`, `admin.module.ts`), `apps/api/src/notifications/notifications.service.ts` (`notifyVendorSuspended`/`notifyVendorReinstated`/`notifyCustomerVendorUnderReview`), `apps/web/src/screens/AdminScreen.tsx` (inline suspend-with-reason prompt) and `VendorAreaScreen.tsx` (reason shown in the onboarding/attention banner). Migration `0027_vendor_suspension_reason.sql`.
